...
impl Hypervisor {
    ...
    fn api_vm_get_id(&self, current: &VCpu) -> spci_vm_id_t {
        unsafe { (*current.vm).id }
    }

    /// Returns the number of VMs configured to run.
    fn api_vm_get_count(&self) -> spci_vm_count_t {
    ...
    fn api_vcpu_get_count(&self, vm_id: spci_vm_id_t, current: &VCpu) -> Option<spci_vcpu_count_t> {
    ...
    fn api_mailbox_writable_get(&self, current: &VCpu) -> Option<spci_vm_id_t> {
        let vm = unsafe { &*current.vm };
    ...
    fn api_mailbox_waiter_get(&self, vm_id: spci_vm_id_t, current: &VCpu) -> Option<spci_vm_id_t> {
    ...
    fn api_interrupt_enable(&self, intid: intid_t, enable: bool, current: &VCpu) -> Result<(), ()> {
    ...
    /// interrupts.
    fn api_interrupt_get(&self, current: &VCpu) -> intid_t {
    ...
    fn api_debug_log(&self, c: c_char, current: &VCpu) {
        let vm = unsafe { &*current.vm };
        vm.debug_log(c);
    }
}
...
pub unsafe extern "C" fn api_vcpu_get_count(
    ...
    hafnium().api_vcpu_get_count(vm_id, &*current).unwrap_or(0)
}
...
pub unsafe extern "C" fn api_interrupt_get(current: *const VCpu) -> intid_t {
    hafnium().api_interrupt_get(&*current)
}
